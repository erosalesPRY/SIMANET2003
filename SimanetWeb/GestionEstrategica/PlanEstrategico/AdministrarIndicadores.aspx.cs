...
{
	public class AdministrarIndicadores : System.Web.UI.Page, IPaginaBase
	{
		#region Controles
		...
		//OTROS
		const string URLMODIFICAR = "DetalleAdministrarIndicadores.aspx?";
		...
		private void Eliminar()
		{
			...
			else
			{
				...
				if(oCPEIndicador.Eliminar(Convert.ToInt32(hCodigo.Value),null) > 
		...
		{
			CPEIndicador oCPEIndicador= new CPEIndicador();
			return oCPEIndicador.ListarIndicadoresPorActividad(
				Convert.ToInt32(Page.Request.QueryString[KEYIDACTIVIDAD]));
		}
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				+ KEYIDOBJESP + Utilitario.Constantes.SIGNOIGUAL + Page.Request.QueryString[KEYIDOBJESP]));										

				e.Item.Cells[COLUMNANUMERACION].Text = Helper.ObtenerIndicedeRegistro(grid.CurrentPageIndex,grid.PageSize,e.Item.ItemIndex);
				Helper.SeleccionarItemGrillaOnClickMoverRaton(e
					,Utilitario.Helper.MostrarDatosEnCajaTexto("hCodigo",dr[Enumerados.IndicadoresColumnas.IdIndicador.ToString()].ToString()));
		...
		{
			this.lblActividad.Text = Page.Request.QueryString[NOMBREACTIVIDAD];
		}
