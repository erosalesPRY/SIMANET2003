...
{
	public class ConsultarPlanEstrategicoMetasAnuales : System.Web.UI.Page, IPaginaBase
	{
		#region Constante
		...
		const string LABELTOTALECONOMICO ="lblTotalEconomico";
		const string LABELTOTALFISICO ="lblTotalFisico";

		const string GRILLAVACIA="No Existen datos Plan Estratégico";
		const string TITULO = "Consultar Plan Estratégico SIMA-PERU S.A. ";
		const string TITULOSECUNDARIO = "SECRETO POR SER DERIVADO DEL PLAN ESTRATEGICO DEL SECTOR DEFENSA Y " +
			"DE LA MARINA DE GUERRA (LEY 27927 ARTICULO 15)";
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			{
				for(int anho=1; anho<=5; anho++)
				{
					Label lbl=(Label) e.Item.Cells[3].FindControl("lblAno"+anho.ToString());
					lbl.Text = Convert.ToString(DateTime.Now.Year + anho);
				}
			}
		...
		{
			lblPagina.Text = TITULO + Convert.ToString(DateTime.Now.Year + 1) + Utilitario.Constantes.ESPACIO +
				Utilitario.Constantes.SIGNOMENOS + Utilitario.Constantes.ESPACIO + Convert.ToString(DateTime.Now.Year + 5);
		...
		private void gridObjetivosEspecificos_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				DataTable dt = (new CPEIndicador()).ConsultarIndicadoresPorObjetivoEspecifico(
					Convert.ToInt32(e.Item.Cells[0].Text));
		...
		private void gridIndicadores_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				{
					Monto = Convert.ToDouble(dr[Enumerados.PlanEstrategicoColumnas.Periodo1.ToString()]) +
							Convert.ToDouble(dr[Enumerados.PlanEstrategicoColumnas.Periodo2.ToString()]) +
							Convert.ToDouble(dr[Enumerados.PlanEstrategicoColumnas.Periodo3.ToString()]) +
							Convert.ToDouble(dr[Enumerados.PlanEstrategicoColumnas.Periodo4.ToString()]) +
							Convert.ToDouble(dr[Enumerados.PlanEstrategicoColumnas.Periodo5.ToString()]);
