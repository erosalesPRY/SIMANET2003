...
{
	public class ConsultarProyectosPorCentroLinea : System.Web.UI.Page,IPaginaBase
	{
		#region Constantes
		const string GRILLAVACIA ="No existe ningún Registro.";  
		const string URLCONSULTAROTS="ConsultarOTs_PorProyecto.aspx?";
		const string COLORDENAMIENTO = "PROYECTO";
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if(!Page.IsPostBack)
			{
				try
				{
					...
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Proyectos",this.ToString(),"Se consultó Proyectos Directorio",Enumerados.NivelesErrorLog.I.ToString()));
					this.LlenarCabecera();
		...
		{
			if (Page.Request[KEYFLAGLN_CO].ToString() == "CO")
			{
			...
			else
			{
				...
				lblTitulo.Text = Page.Request[KEYLINEA].ToString().ToUpper();
			}
		}
		...
		private DataTable ObtenerDatos()
		{
			...
			DataTable dtGeneral =oCProyectos.ConsultarProyectosPorCO_LN(Page.Request.Params[KEYFLAGLN_CO].ToString(), 
				Page.Request.Params[KEYIDCENTRO].ToString(),
				Page.Request.Params[KEYIDLINEA].ToString());			
			return dtGeneral;
		}
		...
		public void LlenarGrillaOrdenamientoPaginacion(string columnaOrdenar, int indicePagina)
		{
			...
			if(dtGeneral!=null)
			{
				...
				grid.Columns[Utilitario.Constantes.POSICIONINDEXTRES].FooterText = Utilitario.Constantes.TEXTOFOOTERTOTAL + Utilitario.Constantes.ESPACIO + dwGeneral.Count.ToString();
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				Helper.ConfiguraColumnaHyperLink(e.Item.Cells[0],grid.CurrentPageIndex,grid.PageSize,e.Item.ItemIndex,
					Helper.HistorialIrAdelantePersonalizado("hGridPaginaSort","hGridPagina","txtBuscar"),
					Helper.MostrarVentana(URLCONSULTAROTS,
					KEYIDPROYECTO + Utilitario.Constantes.SIGNOIGUAL + dr[Enumerados.INTColumna_PRY_OT_SEDES.COD_PROYECTO.ToString()].ToString()
					+ Utilitario.Constantes.SIGNOAMPERSON 
					+ KEYPROYECTO + Utilitario.Constantes.SIGNOIGUAL + dr[Enumerados.INTColumna_PRY_OT_SEDES.PROYECTO.ToString()].ToString()
					+ Utilitario.Constantes.SIGNOAMPERSON 
					+ KEYCENTRO + Utilitario.Constantes.SIGNOIGUAL + dr[Enumerados.INTColumna_PRY_OT_SEDES.CENTRO_OPERATIVO.ToString()].ToString()
					+ Utilitario.Constantes.SIGNOAMPERSON 
					+ KEYLINEA + Utilitario.Constantes.SIGNOIGUAL + dr[Enumerados.INTColumna_PRY_OT_SEDES.LIN_NEG.ToString()].ToString()
					+ Utilitario.Constantes.SIGNOAMPERSON 
				...
				//((Label)e.Item.Cells[5].FindControl(CAMPO_PRO_MAT)).Text= Convert.ToDouble(dr[Enumerados.INTColumna_PRY_OT_SEDES.PROG_MAT.ToString()]).ToString(Utilitario.Constantes.FORMATODECIMAL4);
