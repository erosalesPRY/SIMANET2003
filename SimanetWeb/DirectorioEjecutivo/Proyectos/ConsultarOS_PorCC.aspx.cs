...
{
	public class ConsultarOS_PorCC : System.Web.UI.Page,IPaginaBase
	{
		#region Constantes
		const string GRILLAVACIA ="No existe ningún Registro.";  
		const string COLORDENAMIENTO = "DES_RCS";
		...
		{
			lblTitulo.Text = Page.Request[KEYCENTRO].ToString().ToUpper() + Utilitario.Constantes.SEPARADORLINEA; 
		}
		...
		private DataTable ObtenerDatos()
		{
			...
			DataTable dtGeneral =oCResumenOCompraOServicio.ConsultarOrdenesServicio_PorCC(Page.Request.Params[KEYIDCENTRO].ToString());
			return dtGeneral;
		}
		private void Totaliza(DataView dv)
		{
			...
			arrTotaliza.Add((object)dv.Table.Compute("SUM(TOT_OSE_SOLES)",dv.RowFilter.ToString()));
			arrTotaliza.Add((object)dv.Table.Compute("SUM(CNT_RCP_OSE)",dv.RowFilter.ToString()));
			arrTotaliza.Add((object)dv.Table.Compute("SUM(CNT_ATE)",dv.RowFilter.ToString()));
		}
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			if(e.Item.ItemType == ListItemType.Footer)
			{
				...
				{
					e.Item.Cells[9].Text = Convert.ToDouble(arrTotaliza[0]).ToString(Utilitario.Constantes.FORMATODECIMAL4);
					e.Item.Cells[10].Text = Convert.ToDouble(arrTotaliza[1]).ToString(Utilitario.Constantes.FORMATODECIMAL4);
					e.Item.Cells[11].Text = Convert.ToDouble(arrTotaliza[2]).ToString(Utilitario.Constantes.FORMATODECIMAL4);
					e.Item.Cells[12].Text = Convert.ToDouble(arrTotaliza[3]).ToString(Utilitario.Constantes.FORMATODECIMAL4);
				}
			}		
			#endregion
		}
