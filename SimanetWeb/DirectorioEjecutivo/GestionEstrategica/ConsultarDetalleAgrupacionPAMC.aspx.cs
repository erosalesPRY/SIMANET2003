...
{
	public class ConsultarDetalleAgrupacionPAMC : System.Web.UI.Page,IPaginaBase
	{
		#region Controles
		...
		protected projDataGridWeb.DataGridWeb gridBitacora;
		protected System.Web.UI.WebControls.Label lblTituloBitacora;
		protected System.Web.UI.WebControls.Label lblResultadoBitacora;
		protected System.Web.UI.HtmlControls.HtmlTable tblBitacora;
		...
		const string DETALLE = "ConsultarTerminosReferenciaPAMC.aspx?";
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				e.Item.Cells[0].Attributes.Add(Utilitario.Constantes.EVENTOCLICK,Utilitario.Constantes.HISTORIALADELANTE + ";" +
					...
					Convert.ToString(dr["idDetalleAgrupacionPAMC"]) +  
				...
				Helper.SeleccionarItemGrillaOnClickMoverRaton
					...
					Utilitario.Helper.MostrarDatosEnCajaTexto
					(txtObservaciones.ID,dr["observaciones"].ToString()),
					Utilitario.Helper.MostrarDatosEnCajaTexto
					(txtDescripcion.ID,dr["descripcion"].ToString()),
					Utilitario.Helper.MostrarDatosEnCajaTexto
					(hNombre.ID,dr["Nombre"].ToString()),
					Utilitario.Helper.MostrarDatosEnCajaTexto
					(txtSituacionActual.ID,dr["SITUACIONACTUAL"].ToString())
					);

				System.Web.UI.WebControls.Image ibtnArchivo = (System.Web.UI.WebControls.Image)e.Item.Cells[4].FindControl("ImgArchivo");	
		...
		{
			tblBitacora.Visible = true;
			LlenarGrillaOrdenamientoPaginacionBitacora("FechaBitacoraProyectoAMC",0);
		...
		private void ibtnFiltrar_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			ltlMensaje.Text = Utilitario.Helper.ElaborarFiltro(this.ObtenerDatos(),
				"nombre"+";Nombre",
				"CodigoSNIP"+";cod.snip",
				"FechaInscripcionSNIP"+";Fec.Ins.Snip",
				"OE"+";OE",
				Utilitario.Constantes.SIGNOASTERISCO + "CO"+";CO",
				"AVANCE"+";Avance"
				);
		...
		public void LlenarGrillaOrdenamientoPaginacionBitacora(string columnaOrdenar,int indicePagina)
		{
			CProyectosApoyoMejoraCompetitibidad oProyectosApoyoMejoraCompetitibidad  = new  CProyectosApoyoMejoraCompetitibidad ();
			DataTable dtNivel = oProyectosApoyoMejoraCompetitibidad.ListarBitacoraProyectoAMC(Convert.ToInt32(hCodigo.Value));
			...
			if(dtNivel!=null)
			{
				...
				if(dwNivel.Count>0)
				{
					...
					gridBitacora.DataSource = dwNivel;
					gridBitacora.CurrentPageIndex = indicePagina;
					gridBitacora.Columns[Utilitario.Constantes.POSICIONINDEXUNO].FooterText = dwNivel.Count.ToString();
					lblTituloBitacora.Text = "BITACORA DE " + hNombre.Value;
				}
				else
				{
					gridBitacora.DataSource = null;
					lblResultadoBitacora.Visible = true;
					lblResultadoBitacora.Text = GRILLAVACIA;
					lblTituloBitacora.Visible = false;
				}
				
			}
			else
			{
				gridBitacora.DataSource = dtNivel;
				lblResultadoBitacora.Text = GRILLAVACIA;
			}
			...
			{
				gridBitacora.CurrentPageIndex = Utilitario.Constantes.ValorConstanteCero;
				gridBitacora.DataBind();
			}
		}
