...
namespace SIMA.SimaNetWeb.DirectorioEjecutivo.Directorio
{
	...
	/// </summary>
	public class AdministracionTemasDirectorio : System.Web.UI.Page,IPaginaBase
	{
		#region Controles
		...
		//Ordenamiento
		const string COLORDENAMIENTO = "Fuente";
		...
		//Paginas
		const string URLDETALLE = "DetalleTemasDirectorio.aspx?";
		const string URLIMPRESION = "PopupImpresionAdministracionTemasDirectorio.aspx";
		...
		//Otros
		const string GRILLAVACIA ="No existe ningún Tema del Directorio.";
		...
		private void eliminar()
		{
			...
			{
				ltlMensaje.Text = Helper.MensajeAlert("Seleccione Tema a Eliminar");
			}
			else
			{
				CMantenimientos oCMantenimientos = new CMantenimientos();

				if(oCMantenimientos.Eliminar(Convert.ToInt32(hCodigo.Value),CNetAccessControl.GetIdUser(),Enumerados.ClasesTAD.TemasDirectorioTAD.ToString())>0)
				{
					//Graba en el Log la acción ejecutada
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Directorio",this.ToString(),"Se eliminó el Tema de Directorio Nro. " + hCodigo.Value +"." ,Enumerados.NivelesErrorLog.I.ToString()));
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				try
				{
					...
					//Graba en el Log la acción ejecutada
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Secretaria - Directorio",this.ToString(),"Se consultó los Temas del Directorio.",Enumerados.NivelesErrorLog.I.ToString()));
		...
		public void LlenarGrillaOrdenamientoPaginacion(string columnaOrdenar,int indicePagina)
		{
			...
			DataTable dtTemasDirectorio =  oCTemasDirectorio.ConsultarTemasPorSituacion(
				...
				Utilitario.Constantes.TODOSTEMAS);
			...
			if(dtTemasDirectorio!=null)
			{
				...
				oCImpresion.GuardarDataImprimirExportar(dtTemasDirectorio,Helper.ObtenerValorString(Enumerados.SeccionesArchivoConfiguracion.ConfiguracionesAplicativo.ToString(),Utilitario.Constantes.CODIGOTITULOREPORTEACUERDOSESION),columnaOrdenar,indicePagina);*/
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				Helper.SeleccionarItemGrillaOnClickMoverRaton(e,
					...
					Utilitario.Helper.MostrarDatosEnCajaTexto("hDescripcion",dr[Enumerados.ColumnasTemasDirectorio.Tema.ToString()].ToString()));
				...
				e.Item.Cells[0].Attributes.Add(Utilitario.Constantes.EVENTOCLICK,
					...
					Convert.ToString(dr[Enumerados.ColumnasTemasDirectorio.IdTema.ToString()]) +  
				...
				e.Item.Cells[5].Text = Convert.ToString(dr[Enumerados.ColumnasTemasDirectorio.Tema.ToString()]).ToUpper();
