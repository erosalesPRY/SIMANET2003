...
namespace SIMA.SimaNetWeb.DirectorioEjecutivo.Directorio
{
	...
	/// </summary>
	public class AdministracionPedidoSesionDirectorio : System.Web.UI.Page,IPaginaBase
	{
		#region Controles
		...
		const string BOTONOBSERVACION = "ibtnDisposicion";
		//Paginas
		...
		//Otros
		const string GRILLAVACIA ="No existe ningún Pedido de Sesión de Directorio.";  
		...
		private void eliminar()
		{
			...
			else
			{
				...
				if(oCMantenimientos.Eliminar(Convert.ToInt32(hCodigo.Value),CNetAccessControl.GetIdUser(),Enumerados.ClasesTAD.PedidoSesionDirectorioTAD.ToString())>0)
				{
					...
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Legal",this.ToString(),"Se eliminó el Pedido de Sesión de Directorio Nro. " + hCodigo.Value +"." ,Enumerados.NivelesErrorLog.I.ToString()));
		...
		public void LlenarGrillaOrdenamientoPaginacion(string columnaOrdenar,int indicePagina)
		{
			...
			DataTable dtPedidoSesionDirectorio =  oCPedidoSesionDirectorio.ConsultarPedidosPorSituacion(
				Convert.ToInt32(Helper.RetornaSessionParaDirectorio()),Convert.ToInt32(ddlbSituacion.SelectedValue));
			...
			if(dtPedidoSesionDirectorio!=null)
			{
				...
				oCImpresion.GuardarDataImprimirExportar(dtPedidoSesionDirectorio,Helper.ObtenerValorString(Enumerados.SeccionesArchivoConfiguracion.ConfiguracionesAplicativo.ToString(),Utilitario.Constantes.CODIGOTITULOREPORTEPEDIDOS),columnaOrdenar,indicePagina);
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				e.Item.Cells[0].Attributes.Add(Utilitario.Constantes.EVENTOCLICK,
					...
					Convert.ToString(dr[Enumerados.ColumnasPedidoSesionDirectorio.IdPedidoSesionDirectorio.ToString()]) + 
				...
				ImageButton img=(ImageButton)e.Item.Cells[5].FindControl(BOTONOBSERVACION);
				img.Attributes.Add(Utilitario.Constantes.EVENTOCLICK,Helper.MostrarVentaModalTextoHTML("",
					Convert.ToString(dr[Enumerados.ColumnasPedidoSesionDirectorio.Observacion.ToString()]),500,400,0,400));
