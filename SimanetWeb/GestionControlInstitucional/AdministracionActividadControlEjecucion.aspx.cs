...
namespace SIMA.SimaNetWeb.GestionControlInstitucional
{
	...
	/// </summary>
	public class AdministracionActividadControlEjecucion : System.Web.UI.Page,IPaginaBase
	{
		#region Controles
		...
		const string TITULO = "ADMINISTRACION DE LA EJECUCION DE LAS ACTIVIDADES DE CONTROL";
		...
		const string DENOMINACION="Denominacion";
		const string METAPROGRAMADA="Meta Programada";
		const string AVANCEPROGRAMADO="Avance Programado";
		const string UNIDADESEJECUTADAS="Unidades Ejecutadas";
		const string AVANCEEJECUTADO="Avance Ejecutado";
		...
		{			
			...
			else
			{
				...
				if(oCMantenimientos.Eliminar(Convert.ToInt32(hCodigo.Value),CNetAccessControl.GetIdUser(),Enumerados.ClasesTAD.ActividadCtrlEjecTAD.ToString())>0)
				{
					//Graba en el Log la acción ejecutada
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Gestion Comercial",this.ToString(),"Se eliminó la Ejecucion de la Actividad de Control Nro. " + hCodigo.Value +"." ,Enumerados.NivelesErrorLog.I.ToString()));
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				Helper.SeleccionarItemGrillaOnClickMoverRaton(e,Utilitario.Helper.MostrarDatosEnCajaTexto("hCodigo",dr[Enumerados.ColumnasActividadCtrlEjec.IdActividadCtrlEjec.ToString()].ToString()));
		...
		private void ibtnFiltrar_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			ltlMensaje.Text= Utilitario.Helper.ElaborarFiltro(dtOCI
				...
				,Utilitario.Enumerados.ColumnasActividadCtrlEjec.NroMetaProgramada.ToString()+ ";METAPROGRAMADA"
				,Utilitario.Enumerados.ColumnasActividadCtrlEjec.PorcentajeAvanceProgramado.ToString()+ ";% AVANCEPROGRAMADO"
				,Utilitario.Enumerados.ColumnasActividadCtrlEjec.NroUnidadesEjecutadas.ToString()+ ";UNIDADESEJECUTADAS"
				,Utilitario.Enumerados.ColumnasActividadCtrlEjec.PorcentajeAvanceEjecutado.ToString()+ ";% AVANCEEJECUTADO"
				,Utilitario.Constantes.SIGNOASTERISCO + Utilitario.Enumerados.ColumnasActividadCtrlEjec.Estado.ToString()+ ";ESTADO");
		}
