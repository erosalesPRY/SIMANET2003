...
{
	/// <summary>
	/// Summary description for AdministracionActividadCtrl.
	/// </summary>
	public class AdministracionActividadCtrl : System.Web.UI.Page, IPaginaBase
	{
		#region Controles
		...
		//Ordenamiento
		const string COLORDENAMIENTO = "IdActividadCtrl Asc";
		...
		//Paginas
		const string URLDETALLE   = "DetalleActividadCtrl.aspx?";
		const string URLIMPRESION = "PopupImpresionConsultarActividadCtrl.aspx";
		...
		//Otros
		const string GRILLAVACIA ="No existe ningúna Actividad de Control.";  
		...
		private void eliminar()
		{
			...
			else
			{
				...
				if(oCMantenimientos.Eliminar(Convert.ToInt32(hCodigo.Value),CNetAccessControl.GetIdUser(),Enumerados.ClasesTAD.ActividadCtrlTAD.ToString())>0)
				{
					//Graba en el Log la acción ejecutada
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Detalle Actividad de Control",this.ToString(),"Se eliminó el Detalle del Área Crítica Nro. " + hCodigo.Value +".", Enumerados.NivelesErrorLog.I.ToString()));
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if(!Page.IsPostBack)
			{
				try
				{
					...
					//Graba en el Log la acción ejecutada
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Actividades de Control ",this.ToString(),"Se consultó las Acciones de Control Posterior.",Enumerados.NivelesErrorLog.I.ToString()));
		...
		private DataTable ObtenerDatos()
		{
			...
			return oCActividadCtrl.ListarActividadCtrl(Convert.ToInt32(Page.Request.QueryString[KEYQPRG]));
		}
		...
		public void LlenarGrillaOrdenamientoPaginacion(string columnaOrdenar, int indicePagina)
		{
			...
			if (dt!=null)
			{
				...
				oCImpresion.GuardarDataImprimirExportar(dt,Helper.ObtenerValorString(Enumerados.SeccionesArchivoConfiguracion.ConfiguracionesAplicativo.ToString(),Utilitario.Constantes.CODIGOTITULOREPORTEACTIVIDADCTRL) + Utilitario.Constantes.ESPACIO + Page.Request.QueryString[KEYQTIT].ToString().ToUpper() ,columnaOrdenar,indicePagina);
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				Helper.SeleccionarItemGrillaOnClickMoverRaton(e,
					Utilitario.Helper.MostrarDatosEnCajaTexto("hCodigo",dr[Enumerados.ColumnasActividadCtrl.IdActividadCtrl.ToString()].ToString()));
				...
				e.Item.Cells[1].Attributes.Add(Utilitario.Constantes.EVENTOCLICK, Utilitario.Constantes.HISTORIALADELANTE + 
					...
					KEYQTIT + Utilitario.Constantes.SIGNOIGUAL + Page.Request.QueryString[KEYQTIT] +  Utilitario.Constantes.SIGNOAMPERSON + 
					KEYQID + Utilitario.Constantes.SIGNOIGUAL + Convert.ToString(dr[Enumerados.ColumnasActividadCtrl.IdActividadCtrl.ToString()]) +  Utilitario.Constantes.SIGNOAMPERSON + 
					Utilitario.Constantes.KEYMODOPAGINA +  Utilitario.Constantes.SIGNOIGUAL + Enumerados.ModoPagina.M.ToString()));

				Helper.SeleccionarItemGrillaOnClickMoverRaton(e,Utilitario.Helper.MostrarDatosEnCajaTexto("hCodigo",dr[Enumerados.ColumnasActividadCtrl.IdActividadCtrl.ToString()].ToString()));
			
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Ene.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaEnero].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Feb.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaFebrero].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Mar.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaMarzo].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Abr.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaAbril].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.May.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaMayo].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Jun.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaJunio].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Jul.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaJulio].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Ago.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaAgosto].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Sep.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaSetiembre].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Oct.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaOctubre].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Nov.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
					e.Item.Cells[PosicionCeldaNoviembre].Text = MESSELECCIONADO;
				}
				if(Convert.ToInt32(dr[Enumerados.ColumnasActividadCtrl.Dic.ToString()]) > Utilitario.Constantes.POSICIONCONTADOR)
				{
		...
		{
			ltlMensaje.Text = Helper.ElaborarFiltro(this.ObtenerDatos(),"CODIGO;CODIGO","DENOMINACION;Denominacion", "UNIDADMEDIDA;UNIDADMEDIDA", "META1ERTRIMESTRE", "META2DOTRIMESTRE", "META3ERTRIMESTRE","META4TOTRIMESTRE");
		}
