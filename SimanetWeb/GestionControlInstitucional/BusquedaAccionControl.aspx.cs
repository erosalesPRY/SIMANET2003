...
namespace SIMA.SimaNetWeb.GestionControlInstitucional
{
	...
	/// </summary>
	public class BusquedaAccionControl : System.Web.UI.Page,IPaginaBase
	{
		#region controles
		...
		protected System.Web.UI.HtmlControls.HtmlInputHidden hAccionControlPosterior;
		...
		//Cadenas
		const string GRILLAVACIA ="No existen ninguna Accion de Control Posterior con la Denominacion ingresada.";

		//JScript
		string JSCERRARVENTANA = "return CerrarVentana();";
		const string TITULO = "BUSQUEDA DE ACCION DE CONTROL POSTERIOR";
		...
		public void LlenarGrillaOrdenamientoPaginacion(string columnaOrdenar, int indicePagina)
		{
			...
			DataTable dtAccionCtrlPosterior =  oCAccionCtrlPosterior.BuscarAccionControlPosterior(this.txtDenominacion.Text, Convert.ToInt32(Page.Request.QueryString[KEYQPRG]));
			
			if(dtAccionCtrlPosterior != null)
			{
				DataView dwAccionCtrlPosterior = dtAccionCtrlPosterior.DefaultView;
				dwAccionCtrlPosterior.Sort = columnaOrdenar;
				grid.DataSource = dtAccionCtrlPosterior; 
		...
		{
			if(hIdAccionControlPosterior.Value.Length==0)
			{
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				Helper.SeleccionarItemGrillaOnClickMoverRaton(e,
					Utilitario.Helper.MostrarDatosEnCajaTexto("hIdAccionControlPosterior",dr[Enumerados.ColumnasAccionCtrlPosterior.IdAccionCtrlPosterior.ToString()].ToString()),
					Utilitario.Helper.MostrarDatosEnCajaTexto("hAccionControlPosterior",dr[Enumerados.ColumnasAccionCtrlPosterior.Denominacion.ToString()].ToString())
					);
			}
		}
	}
