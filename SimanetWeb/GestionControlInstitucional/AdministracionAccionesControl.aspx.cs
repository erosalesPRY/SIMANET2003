...
namespace SIMA.SimaNetWeb.GestionControlInstitucional
{
	...
	/// </summary>
	public class AdministracionAccionesControl: System.Web.UI.Page,IPaginaBase
	{
		#region Controles
		...
		//Ordenamiento
		const string COLORDENAMIENTO = "ObservacionAccion";

		//Paginas
		const string URLDETALLE = "DetalleAccionesControl.aspx?";
		...
		//Otros
		const string GRILLAVACIA ="No existe ninguna Acción Tomada.";  
		...
		private void eliminar()
		{
			...
			else
			{
				...
				if(oCMantenimientos.Eliminar(Convert.ToInt32(hCodigo.Value),CNetAccessControl.GetIdUser(),Enumerados.ClasesTAD.AccionesTomadasTAD.ToString())>0)
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if(!Page.IsPostBack)
			{
				try
				{
					...
					//Graba en el Log la acción ejecutada
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"OCI",this.ToString(),"Se consultó las Acciones Tomadas",Enumerados.NivelesErrorLog.I.ToString()));
		...
		public void LlenarGrillaOrdenamientoPaginacion(string columnaOrdenar, int indicePagina)
		{
			...
			CAccionesTomadas oCAccionesTomadas= new CAccionesTomadas();
			DataTable dtAccionesTomadas =  oCAccionesTomadas.ConsultarAccionesTomadas(
				Convert.ToInt32(Page.Request.QueryString[KEYQIDOBSERVACION]),Convert.ToInt32(Page.Request.QueryString[KEYQIDTIPOSEGUIMIENTO]));
			
			if(dtAccionesTomadas!=null)
			{
				DataView dwAccionesTomadas = dtAccionesTomadas.DefaultView;
				dwAccionesTomadas.Sort = columnaOrdenar ;
				grid.DataSource = dwAccionesTomadas;
				grid.CurrentPageIndex =indicePagina;

				grid.Columns[Utilitario.Constantes.POSICIONFOOTERTOTAL].FooterText = Utilitario.Constantes.TEXTOFOOTERTOTAL;
				grid.Columns[Utilitario.Constantes.POSICIONTOTAL].FooterText = dwAccionesTomadas.Count.ToString();
			}
			else
			{
				grid.DataSource = dtAccionesTomadas;
		...
		private void redireccionaPaginaAgregar()
		{
			Page.Response.Redirect(URLDETALLE + Utilitario.Constantes.KEYMODOPAGINA +  
				...
				Utilitario.Constantes.SIGNOAMPERSON + KEYQIDOBSERVACION + Utilitario.Constantes.SIGNOIGUAL +
				Page.Request.QueryString[KEYQIDOBSERVACION].ToString());
		}
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				Helper.SeleccionarItemGrillaOnClickMoverRaton(e,
					Utilitario.Helper.MostrarDatosEnCajaTexto("hCodigo",dr[Enumerados.ColumnasAccionesTomadas.IdAccionTomada.ToString()].ToString()));
				...
				e.Item.Cells[0].Attributes.Add(Utilitario.Constantes.EVENTOCLICK,
					...
					Convert.ToString(dr[Enumerados.ColumnasAccionesTomadas.IdAccionTomada.ToString()])));
