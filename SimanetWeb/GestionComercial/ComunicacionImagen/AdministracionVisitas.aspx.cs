		...
		{			
			...
			else
			{
				...
				if(oCMantenimientos.Eliminar(Convert.ToInt32(hCodigo.Value),CNetAccessControl.GetIdUser(),Enumerados.ClasesTAD.VisitaTAD.ToString())>0)
				{
					...
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),"Gestion Comercial",this.ToString(),"Se eliminó la Visita Nro. " + hCodigo.Value +"." ,Enumerados.NivelesErrorLog.I.ToString()));
