...
namespace SIMA.SimaNetWeb.GestionComercial.InformacionComercial
{
	...
	public class ConsultarMaterialPorCentroOperativo : System.Web.UI.Page, IPaginaBase
	{
		...
		//Paginas
		const string URLMONTOClIENTEPORCOYLN = "ConsultarDetalleMontoClientePorCOyLN.aspx?";
		const string URLMONTOClIENTEPORLN = "ConsultarDetalleMontoClientePorCO.aspx?";
		const string URLMONTOClIENTEPORCO = "ConsultarDetalleMontoClientePorLN.aspx?";
		const string URLIMPRESION = "PopupImpresionConsultarDetalleMontoCliente.aspx?";
		...
		const int PosicionUndMdd = 3;
		const int PosicionDcp = 4;
		const int PosicionStkGrv = 5;
		const int PosicionPrcPmdSolGrv = 6;
		const int PosicionStkExn = 7;
		const int PosicionPrcPmdSolExn = 8;
		...
		public void LlenarGrilla()
		{
			...
			DataTable dtMaterialPorCentroOperativo = new DataTable();
			CMaterial oCMaterial = new CMaterial();

			dtMaterialesPorCentroOperativo =  oCMaterial.ConsultarMaterialesPorCentroOperativo(Convert.ToInt32('1'),Convert.ToInt32(Page.Request.QueryString[KEYQID]),Convert.ToDateTime(CalFechaInicio.SelectedDate),Convert.ToDateTime(CalFechaFin.SelectedDate));

			if(dtVentasPorCliente!=null)
			{
				dtImpresion = dtMaterialPorCentroOperativo.Copy();
				dtImpresion.TableName = TablaImpresion0;

				DataView dwMateriales =	dtMaterialPorCentroOperativo.DefaultView;
		...
		{
			lblNombreCentroOperativo.Text = Convert.ToString(Page.Request.QueryString[KEYQNOMBRE]);
		}
		...
		private void grid_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				if(e.Item.Cells[Utilitario.Constantes.POSICIONDEFAULT].Text == NombreGlosaTotales)
				{
					...
					e.Item.Cells[PosicionMat].Text = Convert.ToDouble(e.Item.Cells[PosicionMat].Text).ToString();
					e.Item.Cells[PosicionUndMdd].Text = Convert.ToDouble(e.Item.Cells[PosicionUndMdd].Text).ToString();
					e.Item.Cells[PosicionDcp].Text = Convert.ToDouble(e.Item.Cells[PosicionDcp].Text).ToString();
					e.Item.Cells[PosicionStkGrv].Text = Convert.ToDouble(e.Item.Cells[PosicionStkGrv].Text).ToString(Constantes.FORMATODECIMAL3);
					e.Item.Cells[PosicionPrcPmdSolGrv].Text = Convert.ToDouble(e.Item.Cells[PosicionPrcPmdSolGrv].Text).ToString(Constantes.FORMATODECIMAL6);
					e.Item.Cells[PosicionStkExn].Text = Convert.ToDouble(e.Item.Cells[PosicionStkExn].Text).ToString(Constantes.FORMATODECIMAL3);
					e.Item.Cells[PosicionPrcPmdSolExn].Text = Convert.ToDouble(e.Item.Cells[PosicionPrcPmdSolExn].Text).ToString(Constantes.FORMATODECIMAL6);
					e.Item.Cells[PosicionPrcUltCmpSol].Text = Convert.ToDouble(e.Item.Cells[PosicionPrcUltCmpSol].Text).ToString(Constantes.FORMATODECIMAL6);
					e.Item.Cells[PosicionFchUltCmp].Text = Convert.ToDouble(e.Item.Cells[PosicionFchUltCmp].Text).ToString(Constantes.FORMATOFECHA3);
					e.Item.Cells[PosicionFchUltSda].Text = Convert.ToDouble(e.Item.Cells[PosicionFchUltSda].Text).ToString(Constantes.FORMATOFECHA3);
					e.Item.Cells[PosicionUbc].Text = Convert.ToDouble(e.Item.Cells[PosicionUbc].Text).ToString();
				}
				else
				{
					e.Item.Cells[PosicionFchEms].Attributes.Add(Utilitario.Constantes.EVENTOCLICK, Utilitario.Constantes.HISTORIALADELANTE +
					...
					e.Item.Cells[PosicionFchEms].Font.Underline = true;
					e.Item.Cells[PosicionFchEms].ForeColor = System.Drawing.Color.Blue;
					e.Item.Cells[PosicionFchEms].Text = Convert.ToDouble(e.Item.Cells[PosicionCN].Text).ToString(Constantes.FORMATOFECHA3);
