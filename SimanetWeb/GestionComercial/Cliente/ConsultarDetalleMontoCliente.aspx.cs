...
{
	/// <summary>
	/// Summary description for ConsultarDetalleMontoCliente.
	/// </summary>
	public class ConsultarDetalleMontoCliente : System.Web.UI.Page, IPaginaBase
	{
		#region Controles
		...
		const string GRILLAVACIA ="No existe ningún Cliente.";
		const string DEFAULTANO = "01/01/";
		const string NombreGlosaTotales = "TOTAL";
		const int PosicionCN = 2;
		const int PosicionRN = 3;
		const int PosicionAE = 4;
		const int PosicionMM = 5;
		const int PosicionSV = 6;
		const int PosicionTotal = 7;
		...
		public void LlenarGrilla()
		{
			...
			DataTable dtVentasPorCliente = new DataTable();
			CCliente oCCliente = new CCliente();

			dtVentasPorCliente =  oCCliente.ListarMontosVentasRealesPorCliente(Convert.ToInt32(Page.Request.QueryString[KEYQID]),Convert.ToDateTime(CalFechaInicio.SelectedDate),Convert.ToDateTime(CalFechaFin.SelectedDate));
