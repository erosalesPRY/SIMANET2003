...
namespace SIMA.SimaNetWeb.Convenio
{
	...
	/// </summary>
	public class AdministracionActividadOrdenTrabajoComoperpac : System.Web.UI.Page, IPaginaBase , IPaginaMantenimento 
	{
		...
		public void Eliminar()
		{
			if(!NullableString.Parse(this.hCodigo.Value).IsNull)
			{
				...
				int IdActividad = Convert.ToInt32(this.hCodigo.Value);
				this.ReiniciaCampos();
				...
				//int retorno=oCActividadesOrdenTrabajo.Eliminar(IdActividad,CNetAccessControl.GetIdUser());
				int retorno=oCMantenimientos.Eliminar(IdActividad,CNetAccessControl.GetIdUser(),Utilitario.Enumerados.ClasesTAD.ActividadesOrdenTrabajoTAD.ToString());
				if(retorno>Utilitario.Constantes.ValorConstanteCero)
				{
					//Graba en el Log la acción ejecutada
					LogAplicativo.GrabarLogAplicativoArchivo(new LogAplicativo(CNetAccessControl.GetUserName(),Utilitario.Enumerados.NombresdeModulo.ConveniosyProyectos.ToString(),this.ToString(),MENSAJEELIMINO + IdActividad.ToString() + Utilitario.Constantes.SIMBOLOPUNTO ,Enumerados.NivelesErrorLog.I.ToString()));
